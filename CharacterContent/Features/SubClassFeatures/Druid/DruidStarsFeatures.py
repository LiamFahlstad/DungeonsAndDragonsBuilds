...
class StarryForm(Feature):
    def __init__(self):
        super().__init__(name="Starry Form", origin="Circle of the Stars Druid Level 3", activation=FeatureActivation(action_type="bonus_action", duration="10 Minutes or Until Dismissed/Incapacitated", range="10-Foot Radius"), usage_tags=["damage", "heal", "buff"])
...
class CosmicOmen(Feature):
    def __init__(self):
        super().__init__(name="Cosmic Omen", origin="Circle of the Stars Druid Level 6", activation=FeatureActivation(action_type="reaction", duration="Until Next Long Rest", range="30 Feet"), usage_tags=["buff", "control"], uses=FeatureUses(max_uses=MAX_ABILITY_MODIFIER, regain_all_on="long rest", current_formula="Current amount: equal to your Wisdom modifier."))
