...
def generate_monster_class(monster: dict) -> str:
    ...
    damage_vuln_str, unmapped_vuln = format_damage_entries(
        monster.get("damage_vulnerabilities", [])
    )
    damage_resist_str, unmapped_resist = format_damage_entries(
        monster.get("damage_resistances", [])
    )
    damage_immune_str, unmapped_immune = format_damage_entries(
        monster.get("damage_immunities", [])
    )
    cond_immune_str, unmapped_conditions = format_condition_list(
    ...
    for label, unmapped in (
        ...
        if unmapped:
            print(
                f"  ! {name}: unmapped {label} value(s), kept as raw text: {unmapped}"
            )
