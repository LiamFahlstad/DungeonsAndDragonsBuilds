# Auto-generated by generate_monsters.py — do not edit manually
#
# NOTE: Classes in this file (Aboleth, CultistHierophant, CyclopsOracle,
# DeathSlaad, Deva, DireWorg, GuardianNaga, HauntingRevenant, NobleProdigy,
# PerformerLegend, SpyMaster, StoneGolem, WarriorCommander, Yochlol,
# YoungGoldDragon, YoungRedDragon) have been hand-edited to demonstrate the
# structured MonsterAbility subclasses in Combat/Definitions.py (MeleeAttack,
# RangedAttack, MeleeOrRangedAttack, SavingThrowEffect, Amphibious,
# LegendaryResistance, MagicResistance, Regeneration, Multiattack,
# SpiderClimb, WebWalker).
# Re-running generate_monsters.py will overwrite these back to plain
# MonsterAbility(...) calls.
from Combat.Definitions import (
    Alignment,
    Amphibious,
    Condition,
    DamageType,
    DamageTypeEntry,
    DiceType,
    ExtendedCombatantData,
    LegendaryResistance,
    MagicResistance,
    MeleeAttack,
    MeleeOrRangedAttack,
    MonsterAbility,
    MonsterType,
    Multiattack,
    RangedAttack,
    Regeneration,
    SavingThrowEffect,
    Size,
    Skill,
    SpiderClimb,
    WebWalker,
)
from Core.Definitions import Ability


class Aboleth(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Aboleth",
            hp=150,
            ac=17,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 21,
                Ability.DEXTERITY: 9,
                Ability.CONSTITUTION: 15,
                Ability.INTELLIGENCE: 18,
                Ability.WISDOM: 15,
                Ability.CHARISMA: 18,
            },
            saving_throws={
                Ability.DEXTERITY: 3,
                Ability.CONSTITUTION: 6,
                Ability.INTELLIGENCE: 8,
                Ability.WISDOM: 6,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.ABERRATION,
            monster_type_note="",
            alignment=Alignment.LAWFUL_EVIL,
            size=Size.LARGE,
            ac_note="",
            hp_formula="20d10 + 40",
            speed_ground_ft=10,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="swim 40 ft.",
            skills={Skill.HISTORY: 12, Skill.PERCEPTION: 10},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Darkvision 120 ft., Passive Perception 20",
            languages="Deep Speech; telepathy 120 ft",
            traits=[
                Amphibious(creature_name="aboleth"),
                MonsterAbility(
                    name="Eldritch Restoration",
                    description="If destroyed, the aboleth gains a new body in 5d10 days, reviving with all its Hit Points in the Far Realm or another location chosen by the DM.",
                ),
                LegendaryResistance(creature_name="aboleth", uses=3, lair_uses=4),
                MonsterAbility(
                    name="Mucus Cloud",
                    description="While underwater, the aboleth is surrounded by mucus. Constitution Saving Throw: DC 14, each creature in a 5-foot Emanation originating from the aboleth at the end of the aboleth's turn. Failure: The target is cursed. Until the curse ends, the target's skin becomes slimy, the target can breathe air and water, and it can't regain Hit Points unless it is underwater. While the cursed creature is outside a body of water, the creature takes 6 (1d12) Acid damage at the end of every 10 minutes unless moisture is applied to its skin before those minutes have passed.",
                ),
                MonsterAbility(
                    name="Probing Telepathy",
                    description="If a creature the aboleth can see communicates telepathically with the aboleth, the aboleth learns the creature's greatest desires.",
                ),
            ],
            actions=[
                Multiattack(
                    creature_name="aboleth",
                    attacks_text="two Tentacle attacks and uses either Consume Memories or Dominate Mind if available",
                ),
                MeleeAttack(
                    name="Tentacle",
                    attack_bonus=9,
                    reach_ft=15,
                    dice_count=2,
                    dice_type=DiceType.D6,
                    damage_bonus=5,
                    damage_type=DamageType.BLUDGEONING,
                    additional_ruling="If the target is a Large or smaller creature, it has the Grappled condition (escape DC 14) from one of four tentacles.",
                ),
                SavingThrowEffect(
                    name="Consume Memories",
                    ability=Ability.INTELLIGENCE,
                    dc=16,
                    target="one creature within 30 feet that is Charmed or Grappled by the aboleth",
                    dice_count=3,
                    dice_type=DiceType.D6,
                    damage_type=DamageType.PSYCHIC,
                    success_effect="Half damage.",
                    extra_ruling="Failure or Success: The aboleth gains the target's memories if the target is a Humanoid and is reduced to 0 Hit Points by this action.",
                ),
                SavingThrowEffect(
                    name="Dominate Mind (2/Day)",
                    ability=Ability.WISDOM,
                    dc=16,
                    target="one creature the aboleth can see within 30 feet",
                    failure_effect="The target has the Charmed condition until the aboleth dies or is on a different plane of existence from the target. While Charmed, the target acts as an ally to the aboleth and is under its control while within 60 feet of it. In addition, the aboleth and the target can communicate telepathically with each other over any distance. The target repeats the save whenever it takes damage as well as after every 24 hours it spends at least 1 mile away from the aboleth, ending the effect on itself on a success.",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[
                MonsterAbility(
                    name="Lash", description="The aboleth makes one Tentacle attack."
                ),
                MonsterAbility(
                    name="Psychic Drain",
                    description="If the aboleth has at least one creature Charmed or Grappled, it uses Consume Memories and regains 5 (1d10) Hit Points.",
                ),
            ],
            legendary_resistances=3,
            lair_actions=[],
            mythic_actions=[],
        )


class CultistHierophant(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Cultist Hierophant",
            hp=144,
            ac=16,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 14,
                Ability.DEXTERITY: 18,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 13,
                Ability.WISDOM: 16,
                Ability.CHARISMA: 20,
            },
            saving_throws={Ability.WISDOM: 7, Ability.CHARISMA: 9},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.HUMANOID,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="17d8 + 68",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.PERCEPTION: 7, Skill.PERSUASION: 9, Skill.RELIGION: 5},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Passive Perception 17",
            languages="Celestial, Common",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="cultist",
                    attacks_text="three attacks, using Pact Blade or Radiant Ray in any combination",
                ),
                MeleeAttack(
                    name="Pact Blade",
                    attack_bonus=9,
                    reach_ft=5,
                    dice_count=2,
                    dice_type=DiceType.D6,
                    damage_bonus=5,
                    damage_type=DamageType.SLASHING,
                    secondary_dice_count=4,
                    secondary_dice_type=DiceType.D8,
                    secondary_damage_type=DamageType.RADIANT,
                ),
                RangedAttack(
                    name="Radiant Ray",
                    attack_bonus=9,
                    range_ft=120,
                    dice_count=4,
                    dice_type=DiceType.D12,
                    damage_bonus=5,
                    damage_type=DamageType.RADIANT,
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The cultist casts one of the following spells, using Charisma as the spellcasting ability (spell save DC 17):",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class CyclopsOracle(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Cyclops Oracle",
            hp=207,
            ac=16,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 22,
                Ability.DEXTERITY: 11,
                Ability.CONSTITUTION: 20,
                Ability.INTELLIGENCE: 16,
                Ability.WISDOM: 18,
                Ability.CHARISMA: 10,
            },
            saving_throws={Ability.CONSTITUTION: 9, Ability.WISDOM: 8},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.GIANT,
            monster_type_note="",
            alignment=Alignment.CHAOTIC_NEUTRAL,
            size=Size.HUGE,
            ac_note="",
            hp_formula="18d12 + 90",
            speed_ground_ft=40,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.HISTORY: 11, Skill.PERCEPTION: 12},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Truesight 30 ft., Passive Perception 22",
            languages="Giant",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="cyclops",
                    attacks_text="three attacks, using Radiant Strike or Flash of Light in any combination",
                ),
                MeleeAttack(
                    name="Radiant Strike",
                    attack_bonus=10,
                    reach_ft=10,
                    dice_count=3,
                    dice_type=DiceType.D10,
                    damage_bonus=6,
                    damage_type=DamageType.RADIANT,
                ),
                MonsterAbility(
                    name="Flash of Light",
                    description="Ranged Attack Roll: +10, range 120 ft. Hit: 17 (2d10 + 6) Radiant damage, and the target has Disadvantage on attack rolls until the end of the cyclops's next turn.",
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The cyclops casts one of the following spells, requiring no Material components and using Wisdom as the spellcasting ability (spell save DC 16):",
                ),
            ],
            bonus_actions=[],
            reactions=[
                MonsterAbility(
                    name="Portent (Recharge 4–6)",
                    description="Trigger: The cyclops or an ally it can see makes a D20 Test. Response: The cyclops rolls 1d20 and chooses whether to use that roll in place of the d20 rolled for the D20 Test.",
                ),
            ],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class DeathSlaad(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Death Slaad",
            hp=178,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 20,
                Ability.DEXTERITY: 15,
                Ability.CONSTITUTION: 19,
                Ability.INTELLIGENCE: 15,
                Ability.WISDOM: 10,
                Ability.CHARISMA: 19,
            },
            saving_throws={},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.ABERRATION,
            monster_type_note="",
            alignment=Alignment.CHAOTIC_EVIL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="21d8 + 84",
            speed_ground_ft=40,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.ARCANA: 6, Skill.PERCEPTION: 8},
            damage_vulnerabilities=[],
            damage_resistances=[
                DamageTypeEntry(damage_types=[DamageType.ACID], note=""),
                DamageTypeEntry(damage_types=[DamageType.COLD], note=""),
                DamageTypeEntry(damage_types=[DamageType.FIRE], note=""),
                DamageTypeEntry(damage_types=[DamageType.LIGHTNING], note=""),
                DamageTypeEntry(damage_types=[DamageType.THUNDER], note=""),
            ],
            damage_immunities=[],
            condition_immunities=[],
            senses="Blindsight 60 ft., Darkvision 60 ft., Passive Perception 18",
            languages="Common, Slaad; telepathy 60 ft",
            traits=[
                MagicResistance(creature_name="slaad"),
                Regeneration(creature_name="slaad", hp=10),
            ],
            actions=[
                Multiattack(
                    creature_name="slaad",
                    attacks_text="two Chaos Blade attacks",
                ),
                MeleeAttack(
                    name="Chaos Blade",
                    attack_bonus=9,
                    reach_ft=10,
                    dice_count=1,
                    dice_type=DiceType.D12,
                    damage_bonus=5,
                    damage_type=DamageType.SLASHING,
                    secondary_dice_count=3,
                    secondary_dice_type=DiceType.D6,
                    secondary_damage_type=DamageType.NECROTIC,
                    additional_ruling="Until the start of the slaad's next turn, the target has a condition determined by rolling 1d4: on a 1, Charmed; on a 2, Frightened; on a 3, Poisoned; or on a 4, Incapacitated.",
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The slaad casts one of the following spells, requiring no Material components and using Charisma as the spellcasting ability (spell save DC 16):",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Shape-Shift",
                    description="The slaad shape-shifts into a Small or Medium Humanoid, or it returns to its true form. Other than its size, its game statistics are the same in each form. Any equipment it is wearing or carrying isn't transformed.",
                ),
            ],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class Deva(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Deva",
            hp=229,
            ac=17,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 18,
                Ability.DEXTERITY: 18,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 17,
                Ability.WISDOM: 20,
                Ability.CHARISMA: 20,
            },
            saving_throws={Ability.WISDOM: 9, Ability.CHARISMA: 9},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.CELESTIAL,
            monster_type_note="Angel",
            alignment=Alignment.LAWFUL_GOOD,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="27d8 + 108",
            speed_ground_ft=30,
            speed_fly_ft=90,
            speed_climb_ft=None,
            speed_special_rules="hover",
            skills={Skill.INSIGHT: 9, Skill.PERCEPTION: 9},
            damage_vulnerabilities=[],
            damage_resistances=[
                DamageTypeEntry(damage_types=[DamageType.RADIANT], note=""),
            ],
            damage_immunities=[],
            condition_immunities=[
                Condition.CHARMED,
                Condition.EXHAUSTION,
                Condition.FRIGHTENED,
            ],
            senses="Darkvision 120 ft., Passive Perception 19",
            languages="All; telepathy 120 ft",
            traits=[
                MonsterAbility(
                    name="Exalted Restoration",
                    description="If the deva dies outside Mount Celestia, its body disappears, and it gains a new body instantly, reviving with all its Hit Points somewhere in Mount Celestia.",
                ),
                MagicResistance(creature_name="deva"),
            ],
            actions=[
                Multiattack(
                    creature_name="deva",
                    attacks_text="two Holy Mace attacks",
                ),
                MeleeAttack(
                    name="Holy Mace",
                    attack_bonus=8,
                    reach_ft=5,
                    dice_count=1,
                    dice_type=DiceType.D6,
                    damage_bonus=4,
                    damage_type=DamageType.BLUDGEONING,
                    secondary_dice_count=4,
                    secondary_dice_type=DiceType.D8,
                    secondary_damage_type=DamageType.RADIANT,
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The deva casts one of the following spells, requiring no Material components and using Charisma as the spellcasting ability (spell save DC 17):",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Divine Aid (2/Day)",
                    description="The deva casts Cure Wounds, Lesser Restoration, or Remove Curse, using the same spellcasting ability as Spellcasting.",
                ),
            ],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class DireWorg(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Dire Worg",
            hp=147,
            ac=16,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 22,
                Ability.DEXTERITY: 14,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 7,
                Ability.WISDOM: 16,
                Ability.CHARISMA: 8,
            },
            saving_throws={Ability.DEXTERITY: 6, Ability.WISDOM: 7},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.FEY,
            monster_type_note="",
            alignment=Alignment.NEUTRAL_EVIL,
            size=Size.HUGE,
            ac_note="",
            hp_formula="14d12 + 56",
            speed_ground_ft=50,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.PERCEPTION: 11},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Darkvision 120 ft., Passive Perception 21",
            languages="Goblin, Sylvan, Worg",
            traits=[
                MagicResistance(creature_name="worg"),
            ],
            actions=[
                Multiattack(
                    creature_name="worg",
                    attacks_text="three Bite attacks",
                ),
                MonsterAbility(
                    name="Bite",
                    description="Melee Attack Roll: +10, reach 5 ft. Hit: 15 (2d8 + 6) Piercing damage plus 7 (2d6) Poison damage, and the target has the Poisoned condition until the start of the worg's next turn. While Poisoned, the target can't regain Hit Points.",
                ),
                SavingThrowEffect(
                    name="Dreadful Howl (Recharge 5–6)",
                    ability=Ability.WISDOM,
                    dc=16,
                    target="each creature within 30 feet that isn't a worg",
                    dice_count=8,
                    dice_type=DiceType.D8,
                    damage_type=DamageType.PSYCHIC,
                    failure_effect="and the target has the Frightened condition until the start of the worg's next turn",
                    success_effect="Half damage only.",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Warp Step",
                    description="The worg teleports, along with a willing creature of its choice within 5 feet of it, up to 30 feet to an unoccupied space it can see.",
                ),
            ],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class GuardianNaga(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Guardian Naga",
            hp=136,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 19,
                Ability.DEXTERITY: 18,
                Ability.CONSTITUTION: 16,
                Ability.INTELLIGENCE: 16,
                Ability.WISDOM: 19,
                Ability.CHARISMA: 18,
            },
            saving_throws={
                Ability.DEXTERITY: 8,
                Ability.CONSTITUTION: 7,
                Ability.INTELLIGENCE: 7,
                Ability.WISDOM: 8,
                Ability.CHARISMA: 8,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.CELESTIAL,
            monster_type_note="",
            alignment=Alignment.LAWFUL_GOOD,
            size=Size.LARGE,
            ac_note="",
            hp_formula="16d10 + 48",
            speed_ground_ft=40,
            speed_fly_ft=None,
            speed_climb_ft=40,
            speed_special_rules="swim 40 ft.",
            skills={Skill.ARCANA: 11, Skill.HISTORY: 11, Skill.RELIGION: 11},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.POISON], note=""),
            ],
            condition_immunities=[
                Condition.CHARMED,
                Condition.PARALYZED,
                Condition.POISONED,
                Condition.RESTRAINED,
            ],
            senses="Darkvision 60 ft., Passive Perception 14",
            languages="Celestial, Common",
            traits=[
                MonsterAbility(
                    name="Celestial Restoration",
                    description="If the naga dies, it returns to life in 1d6 days and regains all its Hit Points unless Dispel Evil and Good is cast on its remains.",
                ),
            ],
            actions=[
                Multiattack(
                    creature_name="naga",
                    attacks_text="two Bite attacks",
                    extra_ruling="It can replace any attack with a use of Poisonous Spittle.",
                ),
                MeleeAttack(
                    name="Bite",
                    attack_bonus=8,
                    reach_ft=10,
                    dice_count=2,
                    dice_type=DiceType.D12,
                    damage_bonus=4,
                    damage_type=DamageType.PIERCING,
                    secondary_dice_count=4,
                    secondary_dice_type=DiceType.D10,
                    secondary_damage_type=DamageType.POISON,
                ),
                SavingThrowEffect(
                    name="Poisonous Spittle",
                    ability=Ability.CONSTITUTION,
                    dc=16,
                    target="one creature the naga can see within 60 feet",
                    dice_count=7,
                    dice_type=DiceType.D8,
                    damage_type=DamageType.POISON,
                    failure_effect="and the target has the Blinded condition until the start of the naga's next turn",
                    success_effect="Half damage only.",
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The naga casts one of the following spells, requiring no Somatic or Material components and using Wisdom as the spellcasting ability (spell save DC 16):",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class HauntingRevenant(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Haunting Revenant",
            hp=203,
            ac=20,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 20,
                Ability.DEXTERITY: 12,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 16,
                Ability.WISDOM: 18,
                Ability.CHARISMA: 20,
            },
            saving_throws={Ability.CONSTITUTION: 8, Ability.WISDOM: 8},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.UNDEAD,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.GARGANTUAN,
            ac_note="",
            hp_formula="14d20 + 56",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={},
            damage_vulnerabilities=[],
            damage_resistances=[
                DamageTypeEntry(damage_types=[DamageType.NECROTIC], note=""),
                DamageTypeEntry(damage_types=[DamageType.PSYCHIC], note=""),
            ],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.POISON], note=""),
            ],
            condition_immunities=[
                Condition.CHARMED,
                Condition.EXHAUSTION,
                Condition.FRIGHTENED,
                Condition.GRAPPLED,
                Condition.PARALYZED,
                Condition.PETRIFIED,
                Condition.POISONED,
                Condition.PRONE,
                Condition.RESTRAINED,
                Condition.UNCONSCIOUS,
            ],
            senses="Truesight 60 ft., Passive Perception 14",
            languages="Common plus two other languages",
            traits=[
                SavingThrowEffect(
                    name="Haunted Zone",
                    ability=Ability.CONSTITUTION,
                    dc=17,
                    target="any creature that casts a spell while inside the revenant's space",
                    failure_effect="The spell fails and is wasted.",
                ),
                MonsterAbility(
                    name="Undead Restoration",
                    description="If the revenant dies, it revives 24 hours later unless Dispel Evil and Good is cast on its remains. If it revives, it animates another Gargantuan object or structure elsewhere on the same plane of existence; it now looks different but uses the same stat block and returns with all its Hit Points.",
                ),
            ],
            actions=[
                Multiattack(
                    creature_name="revenant",
                    attacks_text="two Object Slam attacks and uses Invitation",
                ),
                MeleeOrRangedAttack(
                    name="Object Slam",
                    attack_bonus=9,
                    attack_bonus_note=" (with Advantage if the target is inside the revenant's space)",
                    reach_ft=10,
                    range_ft=30,
                    long_range_ft=90,
                    dice_count=5,
                    dice_type=DiceType.D8,
                    damage_bonus=5,
                    damage_type=DamageType.BLUDGEONING,
                ),
                SavingThrowEffect(
                    name="Invitation",
                    ability=Ability.CHARISMA,
                    dc=17,
                    target="each creature in a 60-foot Cone",
                    failure_effect="The target is teleported inside the revenant's space and swallowed. A swallowed creature has Total Cover against attacks and other effects outside the revenant. While the revenant has Hit Points, a swallowed creature can leave the revenant's space only by using magic that enables planar travel, such as the Plane Shift spell.",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class NobleProdigy(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Noble Prodigy",
            hp=148,
            ac=16,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 8,
                Ability.DEXTERITY: 16,
                Ability.CONSTITUTION: 12,
                Ability.INTELLIGENCE: 15,
                Ability.WISDOM: 14,
                Ability.CHARISMA: 19,
            },
            saving_throws={
                Ability.DEXTERITY: 7,
                Ability.CONSTITUTION: 5,
                Ability.WISDOM: 6,
                Ability.CHARISMA: 8,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.HUMANOID,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="27d8 + 27",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.PERCEPTION: 6, Skill.PERSUASION: 8},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Passive Perception 16",
            languages="Common plus two other languages",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="noble",
                    attacks_text="three Beguiling Strike attacks",
                ),
                MonsterAbility(
                    name="Beguiling Strike",
                    description="Melee or Ranged Attack Roll: +8, reach 5 ft. or range 60 ft. Hit: 18 (4d6 + 4) Psychic damage, and the target has the Charmed condition until the start of the noble's next turn.",
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The noble casts one of the following spells, requiring no Material components and using Charisma as the spellcasting ability (spell save DC 16):",
                ),
            ],
            bonus_actions=[],
            reactions=[
                MonsterAbility(
                    name="Shield (2/Day)",
                    description="The noble casts Shield in response to that spell's trigger, using the same spellcasting ability as Spellcasting.",
                ),
            ],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class PerformerLegend(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Performer Legend",
            hp=162,
            ac=20,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 12,
                Ability.DEXTERITY: 20,
                Ability.CONSTITUTION: 14,
                Ability.INTELLIGENCE: 15,
                Ability.WISDOM: 16,
                Ability.CHARISMA: 20,
            },
            saving_throws={
                Ability.DEXTERITY: 9,
                Ability.INTELLIGENCE: 6,
                Ability.WISDOM: 7,
                Ability.CHARISMA: 9,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.HUMANOID,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="25d8 + 50",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={
                Skill.ACROBATICS: 13,
                Skill.ATHLETICS: 5,
                Skill.PERCEPTION: 7,
                Skill.PERFORMANCE: 13,
                Skill.STEALTH: 9,
            },
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Passive Perception 17",
            languages="Common plus two other languages",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="performer",
                    attacks_text="three Bejeweled Baton attacks",
                ),
                MeleeAttack(
                    name="Bejeweled Baton",
                    attack_bonus=9,
                    reach_ft=5,
                    dice_count=2,
                    dice_type=DiceType.D4,
                    damage_bonus=5,
                    damage_type=DamageType.BLUDGEONING,
                    secondary_dice_count=3,
                    secondary_dice_type=DiceType.D6,
                    secondary_damage_type=DamageType.PSYCHIC,
                ),
                SavingThrowEffect(
                    name="Majestic Song",
                    ability=Ability.WISDOM,
                    dc=17,
                    target="each creature in a 20-foot-radius Sphere centered on a point within 120 feet",
                    dice_count=4,
                    dice_type=DiceType.D8,
                    damage_bonus=4,
                    damage_type=DamageType.PSYCHIC,
                    failure_effect="and the target has the Charmed or Frightened condition (performer's choice) until the end of the performer's next turn",
                    success_effect="Half damage only.",
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The performer casts one of the following spells, requiring no Material components and using Charisma as the spellcasting ability (spell save DC 17):",
                ),
            ],
            bonus_actions=[],
            reactions=[
                MonsterAbility(
                    name="Warding Charm",
                    description="Trigger: A creature hits the performer with an attack roll. Response—Wisdom Saving Throw: DC 17, the triggering creature. Failure: The attack roll misses the performer, and the target has the Charmed condition until the end of the performer's next turn.",
                ),
            ],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class SpyMaster(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Spy Master",
            hp=137,
            ac=19,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 10,
                Ability.DEXTERITY: 20,
                Ability.CONSTITUTION: 12,
                Ability.INTELLIGENCE: 18,
                Ability.WISDOM: 16,
                Ability.CHARISMA: 16,
            },
            saving_throws={
                Ability.DEXTERITY: 9,
                Ability.CONSTITUTION: 5,
                Ability.INTELLIGENCE: 8,
                Ability.WISDOM: 7,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.HUMANOID,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="25d8 + 25",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=30,
            speed_special_rules="",
            skills={
                Skill.DECEPTION: 7,
                Skill.INSIGHT: 7,
                Skill.INVESTIGATION: 8,
                Skill.PERCEPTION: 11,
                Skill.SLEIGHT_OF_HAND: 9,
                Skill.STEALTH: 13,
            },
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Passive Perception 21",
            languages="Common plus two other languages",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="spy",
                    attacks_text="three attacks, using Rapier or Hand Crossbow in any combination",
                ),
                MeleeAttack(
                    name="Rapier",
                    attack_bonus=9,
                    reach_ft=5,
                    dice_count=2,
                    dice_type=DiceType.D8,
                    damage_bonus=5,
                    damage_type=DamageType.PIERCING,
                    secondary_dice_count=2,
                    secondary_dice_type=DiceType.D6,
                    secondary_damage_type=DamageType.POISON,
                ),
                RangedAttack(
                    name="Hand Crossbow",
                    attack_bonus=9,
                    range_ft=30,
                    long_range_ft=120,
                    dice_count=2,
                    dice_type=DiceType.D6,
                    damage_bonus=5,
                    damage_type=DamageType.PIERCING,
                    secondary_dice_count=2,
                    secondary_dice_type=DiceType.D8,
                    secondary_damage_type=DamageType.POISON,
                ),
                MonsterAbility(
                    name="Smoke Bomb (1/Day)",
                    description="The spy throws a bomb to a point it can see within 30 feet of itself. Constitution Saving Throw: DC 16, each creature in a 20-foot-radius Sphere centered on that point. Failure: 28 (8d6) Poison damage, and the target has the Blinded condition until the end of the spy's next turn. Success: Half damage only.",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Cunning Action",
                    description="The spy takes the Dash, Disengage, or Hide action.",
                ),
            ],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class StoneGolem(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Stone Golem",
            hp=220,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 22,
                Ability.DEXTERITY: 9,
                Ability.CONSTITUTION: 20,
                Ability.INTELLIGENCE: 3,
                Ability.WISDOM: 11,
                Ability.CHARISMA: 1,
            },
            saving_throws={},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.CONSTRUCT,
            monster_type_note="",
            alignment=Alignment.UNALIGNED,
            size=Size.LARGE,
            ac_note="",
            hp_formula="21d10 + 105",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.POISON], note=""),
                DamageTypeEntry(damage_types=[DamageType.PSYCHIC], note=""),
            ],
            condition_immunities=[
                Condition.CHARMED,
                Condition.EXHAUSTION,
                Condition.FRIGHTENED,
                Condition.PARALYZED,
                Condition.PETRIFIED,
                Condition.POISONED,
            ],
            senses="Darkvision 120 ft., Passive Perception 10",
            languages="Understands Common plus two other languages but can't speak",
            traits=[
                MonsterAbility(
                    name="Immutable Form", description="The golem can't shape-shift."
                ),
                MagicResistance(creature_name="golem"),
            ],
            actions=[
                Multiattack(
                    creature_name="golem",
                    attacks_text="two attacks, using Slam or Force Bolt in any combination",
                ),
                MeleeAttack(
                    name="Slam",
                    attack_bonus=10,
                    reach_ft=5,
                    dice_count=2,
                    dice_type=DiceType.D8,
                    damage_bonus=6,
                    damage_type=DamageType.BLUDGEONING,
                    secondary_dice_count=2,
                    secondary_dice_type=DiceType.D8,
                    secondary_damage_type=DamageType.FORCE,
                ),
                RangedAttack(
                    name="Force Bolt",
                    attack_bonus=9,
                    range_ft=90,
                    dice_count=4,
                    dice_type=DiceType.D12,
                    damage_type=DamageType.FORCE,
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Slow (Recharge 5–6)",
                    description="The golem casts the Slow spell, requiring no spell components and using Constitution as the spellcasting ability (spell save DC 17).",
                ),
            ],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class WarriorCommander(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Warrior Commander",
            hp=161,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 21,
                Ability.DEXTERITY: 20,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 14,
                Ability.WISDOM: 16,
                Ability.CHARISMA: 14,
            },
            saving_throws={
                Ability.STRENGTH: 9,
                Ability.DEXTERITY: 9,
                Ability.CONSTITUTION: 8,
                Ability.WISDOM: 7,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.HUMANOID,
            monster_type_note="",
            alignment=Alignment.NEUTRAL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="19d8 + 76",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=None,
            speed_special_rules="",
            skills={Skill.ATHLETICS: 9, Skill.INSIGHT: 7, Skill.PERCEPTION: 7},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[],
            condition_immunities=[],
            senses="Passive Perception 17",
            languages="Common plus one other language",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="warrior",
                    attacks_text="three attacks, using Greatsword or Longbow in any combination",
                ),
                MeleeAttack(
                    name="Greatsword",
                    attack_bonus=9,
                    reach_ft=5,
                    dice_count=4,
                    dice_type=DiceType.D6,
                    damage_bonus=5,
                    damage_type=DamageType.SLASHING,
                    additional_ruling="The warrior also creates one of the following effects:",
                ),
                MonsterAbility(
                    name="Sap",
                    description=". The target has Disadvantage on its next attack roll before the start of the warrior's next turn.",
                ),
                MonsterAbility(
                    name="Maneuver",
                    description=". One ally who can see or hear the warrior can take a Reaction to move up to half the ally's Speed without provoking Opportunity Attacks.",
                ),
                MonsterAbility(
                    name="Longbow",
                    description="Ranged Attack Roll: +9, range 150/600 ft. Hit: 18 (3d8 + 5) Piercing damage, and the target's Speed decreases by 10 feet until the end of the target's next turn.",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Tactical Charge",
                    description="The warrior moves up to half its Speed straight toward an enemy it can see without provoking Opportunity Attacks.",
                ),
            ],
            reactions=[
                MonsterAbility(
                    name="Counterattack",
                    description="Trigger: The warrior is hit by an attack roll. Response: The warrior adds 4 to its AC against that attack, possibly causing it to miss. On a miss, the warrior can make one Greatsword or Longbow attack against the attacker.",
                ),
            ],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class Yochlol(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Yochlol",
            hp=153,
            ac=15,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 15,
                Ability.DEXTERITY: 19,
                Ability.CONSTITUTION: 18,
                Ability.INTELLIGENCE: 13,
                Ability.WISDOM: 15,
                Ability.CHARISMA: 17,
            },
            saving_throws={
                Ability.DEXTERITY: 8,
                Ability.INTELLIGENCE: 5,
                Ability.WISDOM: 6,
                Ability.CHARISMA: 7,
            },
            spell_slots={},
            cr="10",
            monster_type=MonsterType.FIEND,
            monster_type_note="Demon",
            alignment=Alignment.CHAOTIC_EVIL,
            size=Size.MEDIUM,
            ac_note="",
            hp_formula="18d8 + 72",
            speed_ground_ft=30,
            speed_fly_ft=None,
            speed_climb_ft=30,
            speed_special_rules="",
            skills={Skill.DECEPTION: 11, Skill.INSIGHT: 6},
            damage_vulnerabilities=[],
            damage_resistances=[
                DamageTypeEntry(damage_types=[DamageType.COLD], note=""),
                DamageTypeEntry(damage_types=[DamageType.FIRE], note=""),
                DamageTypeEntry(damage_types=[DamageType.LIGHTNING], note=""),
            ],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.POISON], note=""),
            ],
            condition_immunities=[Condition.POISONED],
            senses="Darkvision 120 ft., Passive Perception 12",
            languages="Abyssal, Elvish, Undercommon",
            traits=[
                MonsterAbility(
                    name="Demonic Restoration",
                    description="If the yochlol dies outside the Abyss, its body dissolves, and it gains a new body instantly, reviving with all its Hit Points in the Abyss.",
                ),
                MagicResistance(creature_name="yochlol"),
                SpiderClimb(creature_name="yochlol"),
                WebWalker(creature_name="yochlol", knows_location=False),
            ],
            actions=[
                Multiattack(
                    creature_name="yochlol",
                    attacks_text="two Caustic Lash attacks, and it can use Spellcasting to cast Web or Dominate Person if available",
                ),
                MeleeOrRangedAttack(
                    name="Caustic Lash",
                    attack_bonus=8,
                    reach_ft=10,
                    range_ft=120,
                    dice_count=6,
                    dice_type=DiceType.D6,
                    damage_bonus=4,
                    damage_type=DamageType.ACID,
                ),
                MonsterAbility(
                    name="Spellcasting",
                    description="The yochlol casts one of the following spells, requiring no Material components and using Charisma as the spellcasting ability (spell save DC 15):",
                ),
            ],
            bonus_actions=[
                MonsterAbility(
                    name="Shape-Shift",
                    description="The yochlol shape-shifts into a Medium Humanoid or a Medium spider or back into its true form. Its game statistics are the same in each form. Any equipment it is wearing or carrying isn't transformed.",
                ),
            ],
            reactions=[
                MonsterAbility(
                    name="Toxic Escape",
                    description="Trigger: The yochlol is hit by an attack roll. Response: The yochlol halves the attack's damage to itself (round down), and it teleports to an unoccupied space it can see within 30 feet of itself. Constitution Saving Throw: DC 15, each creature within 5 feet of the yochlol's destination space. Failure: The target has the Poisoned condition until the end of its next turn. While Poisoned, it has the Incapacitated condition.",
                ),
            ],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class YoungGoldDragon(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Young Gold Dragon",
            hp=178,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 23,
                Ability.DEXTERITY: 14,
                Ability.CONSTITUTION: 21,
                Ability.INTELLIGENCE: 16,
                Ability.WISDOM: 13,
                Ability.CHARISMA: 20,
            },
            saving_throws={Ability.DEXTERITY: 6, Ability.WISDOM: 5},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.DRAGON,
            monster_type_note="Metallic",
            alignment=Alignment.LAWFUL_GOOD,
            size=Size.LARGE,
            ac_note="",
            hp_formula="17d10 + 85",
            speed_ground_ft=40,
            speed_fly_ft=80,
            speed_climb_ft=None,
            speed_special_rules="swim 40 ft.",
            skills={
                Skill.INSIGHT: 5,
                Skill.PERCEPTION: 9,
                Skill.PERSUASION: 9,
                Skill.STEALTH: 6,
            },
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.FIRE], note=""),
            ],
            condition_immunities=[],
            senses="Blindsight 30 ft., Darkvision 120 ft., Passive Perception 19",
            languages="Common, Draconic",
            traits=[
                Amphibious(creature_name="dragon"),
            ],
            actions=[
                Multiattack(
                    creature_name="dragon",
                    attacks_text="three Rend attacks",
                    extra_ruling="It can replace one attack with a use of Weakening Breath.",
                ),
                MeleeAttack(
                    name="Rend",
                    attack_bonus=10,
                    reach_ft=10,
                    dice_count=2,
                    dice_type=DiceType.D10,
                    damage_bonus=6,
                    damage_type=DamageType.SLASHING,
                ),
                SavingThrowEffect(
                    name="Fire Breath (Recharge 5–6)",
                    ability=Ability.DEXTERITY,
                    dc=17,
                    target="each creature in a 30-foot Cone",
                    dice_count=10,
                    dice_type=DiceType.D10,
                    damage_type=DamageType.FIRE,
                    success_effect="Half damage.",
                ),
                SavingThrowEffect(
                    name="Weakening Breath",
                    ability=Ability.STRENGTH,
                    dc=17,
                    target="each creature that isn't currently affected by this breath in a 30-foot Cone",
                    failure_effect="The target has Disadvantage on Strength-based D20 Tests and subtracts 3 (1d6) from its damage rolls. It repeats the save at the end of each of its turns, ending the effect on itself on a success. After 1 minute, it succeeds automatically.",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


class YoungRedDragon(ExtendedCombatantData):
    def __init__(self):
        super().__init__(
            combatant_type="Young Red Dragon",
            hp=178,
            ac=18,
            temp_hp=0,
            conditions=[],
            ability_scores={
                Ability.STRENGTH: 23,
                Ability.DEXTERITY: 10,
                Ability.CONSTITUTION: 21,
                Ability.INTELLIGENCE: 14,
                Ability.WISDOM: 11,
                Ability.CHARISMA: 19,
            },
            saving_throws={Ability.DEXTERITY: 4, Ability.WISDOM: 4},
            spell_slots={},
            cr="10",
            monster_type=MonsterType.DRAGON,
            monster_type_note="Chromatic",
            alignment=Alignment.CHAOTIC_EVIL,
            size=Size.LARGE,
            ac_note="",
            hp_formula="17d10 + 85",
            speed_ground_ft=40,
            speed_fly_ft=80,
            speed_climb_ft=40,
            speed_special_rules="",
            skills={Skill.PERCEPTION: 8, Skill.STEALTH: 4},
            damage_vulnerabilities=[],
            damage_resistances=[],
            damage_immunities=[
                DamageTypeEntry(damage_types=[DamageType.FIRE], note=""),
            ],
            condition_immunities=[],
            senses="Blindsight 30 ft., Darkvision 120 ft., Passive Perception 18",
            languages="Common, Draconic",
            traits=[],
            actions=[
                Multiattack(
                    creature_name="dragon",
                    attacks_text="three Rend attacks",
                ),
                MeleeAttack(
                    name="Rend",
                    attack_bonus=10,
                    reach_ft=10,
                    dice_count=2,
                    dice_type=DiceType.D6,
                    damage_bonus=6,
                    damage_type=DamageType.SLASHING,
                    secondary_dice_count=1,
                    secondary_dice_type=DiceType.D6,
                    secondary_damage_type=DamageType.FIRE,
                ),
                SavingThrowEffect(
                    name="Fire Breath (Recharge 5–6)",
                    ability=Ability.DEXTERITY,
                    dc=17,
                    target="each creature in a 30-foot Cone",
                    dice_count=16,
                    dice_type=DiceType.D6,
                    damage_type=DamageType.FIRE,
                    success_effect="Half damage.",
                ),
            ],
            bonus_actions=[],
            reactions=[],
            legendary_actions=[],
            legendary_resistances=0,
            lair_actions=[],
            mythic_actions=[],
        )


__all__ = [
    "Aboleth",
    "CultistHierophant",
    "CyclopsOracle",
    "DeathSlaad",
    "Deva",
    "DireWorg",
    "GuardianNaga",
    "HauntingRevenant",
    "NobleProdigy",
    "PerformerLegend",
    "SpyMaster",
    "StoneGolem",
    "WarriorCommander",
    "Yochlol",
    "YoungGoldDragon",
    "YoungRedDragon",
]
