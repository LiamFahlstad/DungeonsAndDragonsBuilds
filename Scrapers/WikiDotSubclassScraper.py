...
BLOCK_TAGS = ["p", "h1", "h2", "h3", "h4", "table", "ul"]
...
class SubclassParser:
    ...
    def _render_heading(self, heading) -> list[str]:
    ...
    def _render_paragraph(self, p) -> list[str]:
        for br in p.find_all("br"):
            br.replace_with("\n")
        text = p.get_text().strip()
        return [text, ""]

    def _render_list(self, ul) -> list[str]:
    ...
    def get_lines(self) -> list[str]:
        ...
        lines: list[str] = []
        for block in self.content.find_all(BLOCK_TAGS, recursive=False):
            if block.name in ("h1", "h2", "h3", "h4"):
                lines.extend(self._render_heading(block))
            elif block.name == "table":
                lines.extend(self._render_table(block))
            elif block.name == "p":
                lines.extend(self._render_paragraph(block))
            elif block.name == "ul":
                lines.extend(self._render_list(block))
...
if __name__ == "__main__":
    ...
    for page_ref, parser in results.items():
        out_path = REPO_ROOT / slug_to_filename(page_ref)
